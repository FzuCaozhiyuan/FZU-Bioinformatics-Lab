...
def gen_k_record(N_feature, fe: FeatureEvaluator):
    new_fe = FeatureEvaluator(fe.task_name)
    new_fe.train = fe.train
    new_fe.test = fe.test
    results = []
    num = len(fe.records.r_list)
    while len(new_fe) <= num:
        action_list = np.random.randint(2, size=N_feature)
        while sum(action_list) < 2:
            action_list = np.random.randint(2, size=N_feature)
        info(f'{len(new_fe)}/{num}:{action_list}')
        result = new_fe.report_performance(action_list, rp=False, flag='train')
...
def main():
    ...
    if not os.path.exists(f'{base_path}/history/{args.task_name}/fe.r.pkl'):
        with open(f'{base_path}/history/{args.task_name}/fe.pkl', 'rb') as f:
            fe: FeatureEvaluator = pickle.load(f)
            info('gen feature evaluator from random...')
            fe = gen_k_record(fe.ds_size, fe)
        with open(f'{base_path}/history/{args.task_name}/fe.r.pkl', 'wb') as f:
            pickle.dump(fe, f)
    else:
        with open(f'{base_path}/history/{args.task_name}/fe.r.pkl', 'rb') as f:
            info('load feature evaluator from local file')
            fe: FeatureEvaluator = pickle.load(f)
    ...
    opt_path = f'{base_path}/history/{fe.task_name}/best-ours-random.hdf'
    ...
    opt_path_test = f'{base_path}/history/{fe.task_name}/best-ours-random-test.hdf'
    ...
    report_head = 'metric\t ours\t random\t random_test\n'
...
# 589, mice_protein, german_credit,spectf

if __name__ == '__main__':
    task_name = 'openml_589'
    type = 'reg'
    opt_path_d = f'{base_path}/history/{task_name}/best-ours-random.hdf'
    opt_path_a = f'{base_path}/history/{task_name}/best-ours-zero.hdf'
    opt_d = pandas.read_hdf(opt_path_d, 'test')
    opt_a = pandas.read_hdf(opt_path_a, 'test')
    ours_test = pandas.read_hdf(f'{base_path}/history/{task_name}.hdf', key='our_test')
    metrics = MEASUREMENT[type]
    report_head = 'metric\t ours\t -d\t -a\n'
    a_1, b_1, c_1, d_1 = test_task_new(ours_test, type)
    a_2, b_2, c_2, d_2 = test_task_new(opt_d, type)
    a_3, b_3, c_3, d_3 = test_task_new(opt_a, type)
    r1 = [a_1, b_1, c_1, d_1]
    r2 = [a_2, b_2, c_2, d_2]
    r3 = [a_3, b_3, c_3, d_3]
    print(report_head)
