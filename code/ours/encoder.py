...
class BiLSTM(Encoder):
    def __init__(self,
        ...
        super(BiLSTM, self).__init__(layers, vocab_size, hidden_size)

        self.mlp_layers = mlp_layers
        self.mlp_hidden_size = mlp_hidden_size

        self.dropout = nn.Dropout(dropout)
        self.rnn = nn.LSTM(self.hidden_size, self.hidden_size, self.layers, bidirectional=True, batch_first=True, dropout=dropout)
        self.mlp = nn.Sequential()
        for i in range(self.mlp_layers):
            if i == 0:
                self.mlp.add_module('layer_{}'.format(i), nn.Sequential(
...
class BiLSTMVAE(Encoder):
    def __init__(self,
                 ...
                 mlp_dropout,
                 d_latent_dim):
        super(BiLSTMVAE, self).__init__(layers, vocab_size, hidden_size)
        ...
        # VAE components
        self.mu = nn.Linear(hidden_size, d_latent_dim)
        self.logvar = nn.Linear(hidden_size, d_latent_dim)
        ...
        self.regressor = nn.Linear(mlp_hidden_size if mlp_layers > 0 else d_latent_dim, 1)
        self.regressor_r = nn.Linear(mlp_hidden_size if mlp_layers > 0 else hidden_size, 1)
    
    def reparameterize(self, mu, logvar):
        epsilon = 1  # Fixed epsilon for deterministic behavior
        return mu + epsilon * torch.exp(logvar/2)
    ...
    def forward(self, x):
        ...
        # VAE components
        mu, logvar = self.mu(seq_emb), self.logvar(seq_emb)
        out = self.reparameterize(mu, logvar)
        
        out_ = out
        # Performance prediction
        out = self.mlp(out)
        predict_value = torch.sigmoid(self.regressor(out))
        
        # Redundancy prediction
        out_ = self.mlp_r(seq_emb)
        predict_redundancy = torch.sigmoid(self.regressor_r(out_))
        
        # Adapter for RNNEncoder output format
        encoder_hidden = None
